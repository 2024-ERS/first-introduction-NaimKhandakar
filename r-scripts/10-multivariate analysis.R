...
pca<-prcomp(envdat,center=T, scale=T)
pca
summary(pca)
# show the site scorest for axis 1
pca$x
...
biplot(pca)
#add label the axis with explained variation
biplot(pca, xlab="PC1 49%", ylab="PC2 21%")
...
d1
d2<-vegan::vegdist(vegdat,method="bray") #bray-cruits dissimilarity 
d1
...
vegan:: orditorp(nmds_veg,dis="sp",priorty=SpecTotCov,
...
# first axis is 8.0 standard deviations of species responses
...
vegan::ordiplot(dca,display="sites",ces=0.7,type = "text")
vegan::orditorp(dca,display = "species",priorty=SpecTotCov,
                col="red",pcol="red",pch="+", cex=0.8)
##### fit the environmental factors to the dca ordination surface
names(envdat)
ef_dca<-vegan::envfit(dca~elevation_m+clay_cm+floodprob+DistGulley_m
                     +redox5+redox10,data=envdat,na.rm=T)
...
vegan::ordisurf(dca,envdat$clay_cm,add=T,col="darkgreen")

#add a species contour
vegan::ordisurf(dca,vegdat$Fest,add=T,col="darkgreen")
...
#redo with only important uncorelated variables
...
vegan::orditorp(cca2,display="species", priorty=SpecTotCov,
                col="red", pcol="red",pch="+",cex=1.1)
