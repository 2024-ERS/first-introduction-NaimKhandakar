...
orchdat3 |>
  group_by(year)|>
summarise(wa_elevation=weighted.mean(x=elevation_m, w=CountSum, na.rm=T))
...
# y=b0 + b1x + b2*I(x1^2)  # expect that b2 is negative
...
p1+geom_line(data=orchdat3,aes(y=pred5,col=factor(year)),linewidth=1.2)

# account for ecological optimum
m6<-glm(CountSum~elevation_m+I(elevation_m^2), 
        family = poisson(log),
        data=orchdat3)
orchdat3$pred6<-predict(m6,type="response")
p1+geom_line(data=orchdat3,aes(y=pred6,col=factor(year)),linewidth=1.2)
...
p1+geom_line(data=orchdat3,aes(y=pred7,col=factor(year)),linewidth=1.2)
...
p1+geom_line(data=orchdat3,aes(y=pred8,col=factor(year)),linewidth=1.2)
